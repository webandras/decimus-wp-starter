...
class ParentController implements DecimusAdminInterface {
	use OptionQueries;
		...
		register_rest_route(
			...
			array(
				...
				'permission_callback' => function () {
					return ! $this->is_protected_route || current_user_can( 'manage_options' );
				},
			)
		);
	}
		...
		if ( $res ) {
			return new WP_REST_Response(
				array(
					'status'      => 200,
					'message'     => 'OK',
					'description' => __( 'Get ' . $this->route . ' settings successful.', self::TEXT_DOMAIN ),
					'data'        => $res,
				),
				200
			);
			...
			return new WP_REST_Response(
				array(
					...
					'description' => __( 'Get ' . $this->route . ' settings failed.', self::TEXT_DOMAIN ),
					'data'        => null,
				),
				400
			);
		...
		if ( $res ) {
			return new WP_REST_Response(
				array(
					'status'      => 200,
					'message'     => 'OK',
					'description' => __( 'Update ' . $this->route . ' settings successful.', self::TEXT_DOMAIN ),
				),
				200
			);
		} else {
			return new WP_REST_Response(
				array(
					'status'      => 400,
					'message'     => 'rest_bad_request',
					'description' => __( 'Update ' . $this->route . ' settings failed.', self::TEXT_DOMAIN ),
				),
				400
			);
		}
